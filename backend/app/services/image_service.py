...
import uuid
import sys
import os  # ⬅️ Agrega esta importación para leer variables de entorno
from typing import Dict, List, Optional
from pathlib import Path
...
class LazyImagePipeline:
    ...
    def is_available(self) -> bool:
        ...
        # ⬇️ Selecciona la ruta del modelo según variable de entorno
        if os.getenv("USE_DOCKER_MODEL_PATH", "0") == "1":
            # Docker/Render: ruta absoluta
            try:
                model_path = Path("/backend/models/CNN_PyTorch/modelo_cnn_stroke_pytorch.zip")
                return model_path.exists()
            except Exception:
            ...
            # Local: ruta relativa al proyecto
            try:
                current_dir = Path(__file__).resolve().parent
                project_root = current_dir.parent.parent.parent
                model_path = project_root / "models" / "CNN_PyTorch" / "modelo_cnn_stroke_pytorch.zip"
...
# ⬅️ Ahora solo cambia USE_DOCKER_MODEL_PATH en tu .env:
# Local: USE_DOCKER_MODEL_PATH=0
# Docker/Render: USE_DOCKER_MODEL_PATH=1
